...
export const SharedPlanetState = ({ planetData, linePos }) => {
    ...
    function updatePlanetPosition(group, posArr, lineArr, realPos) {
        ...
        if (speed == -1) {
            ...
            customData.current.setDate(new Date(realPos.date));
        }
    }
    ...
    useFrame(({ clock }) => {
        ...
        const timeSinceLastRealUpdate =
            clock.elapsedTime - lastRealPositionUpdate.current;
    ...
    const getAllPositions = () => {
        ...
        const fetchData = async () => {
            fetch(
                `http://127.0.0.1:8002/duration` + `?date=${dateTime.current}`,
            ).then((response) => {
                if (!response.ok) {
                    throw new Error(
    ...
    const getRealTimePos = () => {
        const fetchData = async () => {
            fetch(`http://127.0.0.1:8002/now`).then((response) => {
                if (!response.ok) {
                    throw new Error(
                        `This is an HTTP error: The status is ${response.status}`,
                    );
    ...
    return (
        <>
            <PlanetOverlayContext.Provider
                value={{ nameVis, iconVis, speed, pathVis }}
            >
                {!loading && planets}
                {loading && (
                    <Html>
